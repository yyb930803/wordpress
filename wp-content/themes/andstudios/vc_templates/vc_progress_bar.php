...
foreach ($graph_lines_data as $line) {
    ...
    $output .= '<div class="progress-bar vc_single_bar">';
    $output .= '<span class="bar-label">' . $line['label'] . $unit . '</span>';
    if ($max_value > 100.00) {
        $percentage_value = (float) $line['value'] > 0 && $max_value > 100.00 ? round((float) $line['value'] / $max_value * 100, 4) : 0;
    } else {
        $percentage_value = $line['value'];
    }
    $output .= '<span class="bar-meter vc_bar' . esc_attr(implode(' ', $bar_options)) . '" data-percentage-value="' . esc_attr($percentage_value) . '" data-meter="' . esc_attr($line['value']) . '"' . $line['bgcolor'] . '><span class="bar-number">' . $percentage_value . '</span></span>';
    $output .= '</div>';
}

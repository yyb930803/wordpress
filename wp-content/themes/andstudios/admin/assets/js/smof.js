...
// win edit
jQuery(document).ready(function ($) {
    $('body').on('click', '#section-demo_data .button-primary', function (e) {
        var confirm = window.confirm('WARNING: Importing demo content will give you pages, posts, theme options, widgets, sidebars and other settings. This will replicate the live demo. Please make sure you have the WooCommerce plugins installed and activated to receive that portion of the content. Clicking this button will replace your current theme options and widgets. It can also take a minute to complete.');
        ...
        if (confirm) {
            ...
            if (!run) {
                import_contents('data1', $, loading_img);
                run = true;
            }

            var timeOutContent = setInterval(function () {
                if (!run) {
                    import_contents('data' + (act.toString()), $, loading_img);
                    run = true;
                }
            }, 1000);
            ...
            var timeOut = setInterval(function () {
                if (act > _total) {
                    import_end_importer($, loading_img);
                    clearInterval(timeOut);
                    clearInterval(timeOutContent);
                }
            }, 1000);
        }
    });
...
function import_contents(_file, jQuery, loading_img) {
    jQuery.ajax({
        url: ajaxurl,
        type: 'post',
        dataType: 'json',
        ...
        success: function (res) {
            if (act <= _total) {
                var _finish = parseInt((act / (_total + 1)) * 100).toString();
                jQuery('.bg-success').css({'width': _finish + '%'});
                jQuery('.success-import').html(_finish);
                
                if(typeof res.mess !== 'undefined' && res.nofile === 'false') {
                    jQuery('.mess-reponse-import').prepend('<div style="clear:both"><h3>/* ======== ' + _finish  + '% ========= */' + '<\/h3>' + res.mess + '<\/div>');
                }
            }
            act += res.end;
            run = false;
...
function import_end_importer(jQuery, loading_img) {
    ...
    jQuery.ajax({
        ...
        type: 'post',
        data: {
            action: 'nasa_import_end_importer'
        },
        success: function (res) {
            if (res === 'imported') {

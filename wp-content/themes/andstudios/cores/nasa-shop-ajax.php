...
if(!function_exists('elessi_live_search_products')) :
    function elessi_live_search_products() {
	 ...
	 global $wpdb;
	$get_cats = $wpdb->get_results("SELECT wn_terms.name,wn_terms.slug FROM wn_terms INNER JOIN wn_term_taxonomy ON wn_terms.term_id = wn_term_taxonomy.term_id
WHERE wn_term_taxonomy.taxonomy='product_cat'");
foreach ($get_cats as $cats) {
          $mrj_categories[$cats->slug] = $cats->name;
		...
		$catkeyword = $_REQUEST['s'];
		/*if (in_array($capital_keyword, $mrj_categories)){*/
		if (in_array(strtolower($catkeyword), array_map("strtolower", $mrj_categories))){
			$mrj_cat_slug = array_search(strtolower($catkeyword), array_map("strtolower", $mrj_categories));
			$results[] = array( 
			'title' => $catkeyword,
			'url' => 'https://www.winefully.com/cantine/'.$mrj_cat_slug.'/'	
			);	
		}

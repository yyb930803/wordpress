...
require_once NASA_CORE_PLUGIN_PATH . 'admin/importer/parsers.php';
...
if (class_exists('WP_Importer')) {
    ...
    class WP_Import extends WP_Importer {
        ...
        public function import_start($file) {
            if (!is_file($file)) {
                echo '<p><strong>' . esc_html__('Sorry, there has been an error.', 'nasa-core') . '</strong><br />';
                echo esc_html__('The file does not exist, please try again.', 'nasa-core') . '</p>';
                $this->footer();
                die();
            }
        ...
        public function import_end() {
            ...
            echo '<p>' . esc_html__('Done. Please wait for continuing to import data', 'nasa-core') . '</p>';
        ...
        public function handle_upload() {
            ...
            if (isset($file['error'])) {
                ...
                echo '<p><strong>' . esc_html__('Sorry, there has been an error.', 'nasa-core') . '</strong><br />';
                printf(__('The export file could not be found at <code>%s</code>. It is likely that this was caused by a permissions problem.', 'nasa-core'), esc_html($file['file']));
                echo '</p>';
                return false;
            }
            ...
            if ($this->version > $this->max_wxr_version) {
                ...
                printf(__('This WXR file (version %s) may not be supported by this version of the importer. Please consider updating.', 'nasa-core'), esc_html($import_data['version']));
                echo '</strong></p></div>';
            }
            ...
            if (!empty($import_data['authors'])) {
                ...
                foreach ($import_data['posts'] as $post) {
                    ...
                    if (empty($login)) {
                        ...
                        printf(__('Failed to import author %s. Their posts will be attributed to the current user.', 'nasa-core'), esc_html($post['post_author']));
                        echo '</p><br />';
                        continue;
                    }
        ...
        public function import_options() {
            ...
            <form action="<?php echo admin_url('admin.php?import=wordpress&amp;step=2'); ?>" method="post">
                ...
                <?php if (!empty($this->authors)) : ?>
                    <h3><?php esc_html_e('Assign Authors', 'nasa-core'); ?></h3>
                    <p><?php esc_html_e('To make it easier for you to edit and save the imported content, you may want to reassign the author of the imported item to an existing user of this site. For example, you may want to import all the entries as <code>admin</code>s entries.', 'nasa-core'); ?></p>
                    <?php if ($this->allow_create_users()) : ?>
                        <p><?php printf(__('If a new user is created by WordPress, a new password will be randomly generated and the new user&#8217;s role will be set as %s. Manually changing the new user&#8217;s details will be necessary.', 'nasa-core'), esc_html(get_option('default_role'))); ?></p>
                    <?php endif; ?>
                    <ol id="authors">
                ...
                <?php if ($this->allow_fetch_attachments()) : ?>
                    <h3><?php esc_html_e('Import Attachments', 'nasa-core'); ?></h3>
                    <p>
                        <input type="checkbox" value="1" name="fetch_attachments" id="import-attachments" />
                        <label for="import-attachments"><?php esc_html_e('Download and import file attachments', 'nasa-core'); ?></label>
                    </p>
                <?php endif; ?>

                <p class="submit"><input type="submit" class="button" value="<?php esc_attr_e('Submit', 'nasa-core'); ?>" /></p>
            </form>
            <?php
        }
        ...
        public function author_select($n, $author) {
            esc_html_e('Import author:', 'nasa-core');
            ...
            if ($create_users) {
                if ($this->version != '1.0') {
                    esc_html_e('or create new user with login name:', 'nasa-core');
                    $value = '';
                } else {
                    esc_html_e('as a new user:', 'nasa-core');
            ...
            if (!$create_users && $this->version == '1.0')
                esc_html_e('assign posts to an existing user:', 'nasa-core');
            else
                esc_html_e('or assign posts to an existing user:', 'nasa-core');
            wp_dropdown_users(array('name' => "user_map[$n]", 'multi' => true, 'show_option_all' => esc_html__('- Select -', 'nasa-core')));
            ...
            foreach ((array) $_POST['imported_authors'] as $i => $old_login) {
                ...
                if (!empty($_POST['user_map'][$i])) {
                    ...
                    if (!is_wp_error($user_id)) {
                        ...
                        echo '<p class="nasa-error">';
                        printf(__('Failed to create new user for %s. Their posts will be attributed to the current user.', 'nasa-core'), esc_html($this->authors[$old_login]['author_display_name']));
        ...
        public function process_categories() {
            ...
            foreach ($this->categories as $cat) {
                ...
                if (!is_wp_error($id)) {
                    ...
                    echo '<p class="nasa-error">';
                    printf(__('Failed to import category %s', 'nasa-core'), esc_html($cat['category_nicename']));
                    if (defined('IMPORT_DEBUG') && IMPORT_DEBUG)
        ...
        public function process_tags() {
            ...
            foreach ($this->tags as $tag) {
                ...
                if (!is_wp_error($id)) {
                    ...
                    echo '<p class="nasa-error">';
                    printf(__('Failed to import post tag %s', 'nasa-core'), esc_html($tag['tag_name']));
                    if (defined('IMPORT_DEBUG') && IMPORT_DEBUG)
                        echo ': ' . $id->get_error_message();
                    echo '</p><br />';
                    continue;
        ...
        public function process_terms() {
            ...
            foreach ($this->terms as $term) {
                ...
                if (!is_wp_error($id)) {
                    ...
                    echo '<p class="nasa-error">';
                    printf(__('Failed to import %s %s', 'nasa-core'), esc_html($term['term_taxonomy']), esc_html($term['term_name']));
                    if (defined('IMPORT_DEBUG') && IMPORT_DEBUG)
                        echo ': ' . $id->get_error_message();
                    echo '</p><br />';
                    continue;
        ...
        public function process_posts() {
            ...
            foreach ($this->posts as $post) {
                ...
                if (!post_type_exists($post['post_type'])) {
                    echo '<p class="nasa-error">';
                    printf(__('Failed to import &#8220;%s&#8221;: Invalid post type %s', 'nasa-core'), esc_html($post['post_title']), esc_html($post['post_type']));
                    echo '</p>';
                ...
                if ($post_exists && get_post_type($post_exists) == $post['post_type']) {
                    printf(__('%s &#8220;%s&#8221; already exists.', 'nasa-core'), $post_type_object->labels->singular_name, esc_html($post['post_title']));
                    ...
                    if (is_wp_error($post_id)) {
                        echo '<p class="nasa-error">';
                        printf(__('Failed to import %s &#8220;%s&#8221;', 'nasa-core'), $post_type_object->labels->singular_name, esc_html($post['post_title']));
                        if (defined('IMPORT_DEBUG') && IMPORT_DEBUG)
                    ...
                    foreach ($post['terms'] as $term) {
                        ...
                        if (!$term_id) {
                            ...
                            if (!is_wp_error($t)) {
                                ...
                                echo '<p class="nasa-error">';
                                printf(__('Failed to import %s %s', 'nasa-core'), esc_html($taxonomy), esc_html($term['name']));
                                if (defined('IMPORT_DEBUG') && IMPORT_DEBUG)
        ...
        public function process_menu_item($item) {
            ...
            if (!$menu_slug) {
                esc_html_e('Menu item skipped due to missing menu slug', 'nasa-core');
                echo '<br />';
                return;
            }
            ...
            if (!$menu_id) {
                printf(__('Menu item skipped due to invalid menu slug: %s', 'nasa-core'), esc_html($menu_slug));
                echo '<br />';
                return;
            } else {
        ...
        public function process_attachment($post, $url) {
            if (!$this->fetch_attachments)
                return new WP_Error('attachment_processing_error', esc_html__('Fetching attachments is not enabled', 'nasa-core'));
            ...
            if ($info = wp_check_filetype($upload['file']))
                ...
                return new WP_Error('attachment_processing_error', esc_html__('Invalid file type', 'nasa-core'));
        ...
        public function header() {
            echo '<div class="wrap">';
            // screen_icon();
            echo '<h2>' . esc_html__('Import WordPress', 'nasa-core') . '</h2>';
            ...
            if (isset($updates[$basename])) {
                ...
                echo '<div class="error"><p><strong>';
                printf(__('A new version of this importer is available. Please update to version %s to ensure compatibility with newer export files.', 'nasa-core'), $update->update->new_version);
                echo '</strong></p></div>';
            }
        }
        ...
        public function greet() {
            ...
            echo '<p>' . esc_html__('Howdy! Upload your WordPress eXtended RSS (WXR) file and we&#8217;ll import the posts, pages, comments, custom fields, categories, and tags into this site.', 'nasa-core') . '</p>';
            echo '<p>' . esc_html__('Choose a WXR (.xml) file to upload, then click Upload file and import.', 'nasa-core') . '</p>';

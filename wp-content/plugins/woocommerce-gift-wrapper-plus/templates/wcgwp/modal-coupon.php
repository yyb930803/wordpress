<?php 
/**
 *  (DEPRECATED) The template for displaying gift wrap modal content in cart/checkout areas
 *
 * This template can be overridden by copying it to yourtheme/woocommerce/wcgwp/modal-coupon.php
 * 
...
<div id="giftwrap_modal<?php echo $label; ?>" class="giftwrapper_products_modal giftwrapper_products_modal<?php echo $label; ?> fusion-modal modal" tabindex="-1" role="dialog">
    <div class="modal-dialog <?php echo apply_filters( 'wcgwp_modal_size', 'modal-lg'); ?> modal-dialog-centered" role="document">
        <div class="modal-content fusion-modal-content">
            ...
            <form class="giftwrapper_products modal_form wcgwp_form" method="post" action="#">
                <div class="modal-body wcgwp_modal_body">
                    ...
                    <ul class="giftwrap_ul">
                        <?php foreach ( $list as $giftwrapper_product ) {
                            ...
                            if ( $list_count === TRUE ) { 
                                echo '<li class="giftwrap_li' . $show_thumbs_class . '"><input type="radio" name="cart_wcgwp_product" id="' . $giftwrap_label . $label . '" value="' . $giftwrapper_product->ID . '"' . $checked . '>';
                                ...
                                echo '<input type="hidden" name="cart_wcgwp_product" value="' . $giftwrapper_product->ID . '" id="' . $giftwrap_label . $label . '">';
                            }
                            ++$count;
                        } ?>
                    </ul>
                    ...
                    <div class="wc_giftwrap_notes_container">
                        <label for="wcgwp_notes<?php echo $label; ?>"><?php echo apply_filters( 'wcgwp_add_wrap_message', esc_html__( 'Add Gift Wrap Message:', 'woocommerce-gift-wrapper-plus' ) ); ?></label>
                        <textarea name="cart_wcgwp_note" id="wcgwp_notes<?php echo $label; ?>" cols="50" rows="4" maxlength="<?php echo get_option( 'giftwrap_textarea_limit', '1000' ); ?>" class="wc_giftwrap_notes"></textarea>	
                    </div>
                </div>

                <div class="modal-footer wcgwp_modal_footer">
                    <button type="submit" class="button btn alt giftwrap_submit giftwrap_submit_cart replace_wrap fusion-button fusion-button-default fusion-button-default-size" name="giftwrap_btn">
